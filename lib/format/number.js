...
class NumberFormat extends AbstractFormat {
  format(value) {
    ...
    const data = this.i18n.getData().number;
    ...
    let result = intPart.slice(-(data.group.size === 2 ? data.group.size + 1 : data.group.size));
  ...
  parse(value) {
    ...
    const [intPart, fracPart = '0', overflow] = value.replace(sign, '').split(data.decimal.symbol);

    if (overflow || (data.group.symbol && fracPart.indexOf(data.group.symbol) !== -1)) {
      return NaN;
    }

    const groups = data.group.symbol ? intPart.split(data.group.symbol) : [intPart];
